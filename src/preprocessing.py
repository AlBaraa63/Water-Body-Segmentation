...
def load_and_preprocess(image_path, mask_path, stats):
    """
    Full pipeline — includes no-data handling.
...
def compute_global_stats(image_dir, image_files):
    ...
    Correctly handles no-data values BEFORE computing stats.
    """
    num_bands   = 12
...
def normalize_with_stats(image, stats):
    ...
    Includes NaN/Inf safety check after normalizing.
    """
    normalized = np.zeros_like(image, dtype=np.float32)
...
def replace_nodata(image, nodata_value=-9999, verbose=False):
    """
    Replace satellite no-data sentinel values (-9999) with valid data.

    If a band has some valid pixels, replace no-data with the median.
    If a band is fully corrupted, replace with 0.0.
    """
